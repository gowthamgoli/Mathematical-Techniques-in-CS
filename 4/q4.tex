...
Let $x^*$ be the solution to the Linear least squares problem of $Ax = b$ then we know that, $b_R = Ax^*$ and $r_N = b_N$, $r_R = b_R - Ax^* = 0$
\begin{align*}
r &= r_R + r_N\\
\implies 0 &= 0 + r_N\\
\implies r_N &= 0\\
\implies b_N &= 0
\end{align*}
$\therefore b$ can be expressed only in terms of the range space component of $A$ as the null space component of $A$ is 0 i.e. $b = b_R = Ax^*$
\end{solution}
\fi
...
Let $x^*$ be the solution to the Linear least squares problem of $Ax = b$ then we know that, $b_R = Ax^* \implies$  $r_R = b_R - Ax^* = 0$ and $r_N = b_N$.

$\because x^* = 0 \implies b_R = Ax^* = 0$ i.e. $r_R = b_R = 0$ and $r_N = b_N$

$\therefore b$ can be expressed only in terms of the null space component of $A$ as the range space component of $A$ is 0 i.e. $b = b_N$.
...
Let $x_1^*, x_2^*$ be the solutions to the Linear least squares problems of $Ax_1 = b_1$ and $Ax_2 = b_2$ then we know that $b_{R_1} = Ax_1^*$ and $b_{R_2} = Ax_2^*$. But is given that $x_1^* = x_2^* \implies b_{R_1} = b_{R_2}$.
...
\begin{align*}
b_1 &= u_1\alpha_1 + u_2\alpha_2 + u_1\alpha_2\\
b_2 &= u_1\beta_1 + u_2\beta_2 + u_1\beta_2\\
...
\end{align*}
It is clear that the difference is very large. It can be explained as follows

From part(a), we know that $b_1=U \alpha$ and $ b_2 = U \beta $
\begin{align*}
\therefore \norm{b_1 - b_2} &= \norm{U \alpha - U \beta }\\
& \leq \norm{U \alpha} - \norm{U \beta}\\
& \leq \norm{\alpha} - \norm{\beta}	\,\,\,\,\,\,\,\,\, \because \norm{U} = 1 \\
\implies \frac{1}{\norm{b_1-b_2}} &\geq \frac{1}{\norm{\alpha} - \norm{\beta}}\\
\implies  \frac{1}{\norm{b_1-b_2}} &\geq \frac{1}{-4.87 \times 10^{-6}}
\end{align*}  
